...
def liste_posts(request):
    posts = Post.objects.filter(
        is_active=True
    ).select_related('prestatire').prefetch_related('images', 'likes', 'commentaires')
...
def mes_posts(request):
    """
    Retourne tous les posts du prestataire connecté (actifs et inactifs).
    Utilisé pour l'écran 'Mes publications' dans le profil prestataire.
    """
    ...
    posts = Post.objects.filter(
        ...
        is_active=True,
    ).select_related('prestatire').prefetch_related('images', 'likes', 'commentaires').order_by('-date_publication')
...
def detail_post(request, pk):
    try:
        post = Post.objects.select_related(
            'prestatire'
        ).prefetch_related('images', 'likes', 'commentaires').get(pk=pk, is_active=True)
...
def creer_post(request):
    ...
    if 'video' in request.FILES:
        try:
            ...
            if duration > 30:
                cloudinary.uploader.destroy(result.get('public_id'), resource_type='video')
                return Response(
                    {'error': 'La video ne doit pas depasser 30 secondes'},
                    status=status.HTTP_400_BAD_REQUEST
                )
            video_url = result.get('secure_url')
        except Exception as e:
            return Response(
                {'error': f'Erreur upload video : {str(e)}'},
                status=status.HTTP_400_BAD_REQUEST
            )

    if not contenu and not video_url:
        return Response(
            {'error': 'Un post doit contenir du texte ou une video'},
            status=status.HTTP_400_BAD_REQUEST
        )
    ...
    for i, img in enumerate(images):
        try:
            result = cloudinary.uploader.upload(img, folder='posts/images/', resource_type='image')
            PostImage.objects.create(post=post, image=result.get('secure_url'), ordre=i)
        except Exception:
            pass
...
def supprimer_post(request, pk):
    """
    Suppression définitive d'un post par le prestataire propriétaire.
    Supprime aussi les images Cloudinary associées.
    """
    if not request.user.is_prestataire:
        return Response({'error': 'Acces refuse'}, status=status.HTTP_403_FORBIDDEN)

    try:
        post = Post.objects.prefetch_related('images').get(
            pk=pk,
            prestatire=request.user.prestatire_profile,
        )
    except Post.DoesNotExist:
        return Response({'error': 'Post introuvable ou non autorise'}, status=status.HTTP_404_NOT_FOUND)
    ...
    for img in post.images.all():
        if img.image:
            try:
                ...
                url = str(img.image)
                # Format : .../posts/images/nom_fichier.ext
                if '/posts/images/' in url:
                    public_id = 'posts/images/' + url.split('/posts/images/')[-1].rsplit('.', 1)[0]
                    cloudinary.uploader.destroy(public_id, resource_type='image')
            except Exception:
                pass

    # Supprimer la vidéo Cloudinary si présente
    if post.video_url:
        try:
            url = str(post.video_url)
            if '/posts/videos/' in url:
                public_id = 'posts/videos/' + url.split('/posts/videos/')[-1].rsplit('.', 1)[0]
                cloudinary.uploader.destroy(public_id, resource_type='video')
        except Exception:
            pass

    post.delete()
    return Response({'message': 'Post supprime definitvement'}, status=status.HTTP_200_OK)
...
def toggle_like(request, pk):
    ...
    if not created:
        like.delete()
        return Response({'message': 'Like retire', 'liked': False, 'total_likes': post.likes.count()})

    return Response(
        {'message': 'Post aime', 'liked': True, 'total_likes': post.likes.count()},
        status=status.HTTP_201_CREATED
    )
...
def ajouter_commentaire(request, pk):
    ...
    if serializer.is_valid():
        ...
        return Response(CommentSerializer(commentaire).data, status=status.HTTP_201_CREATED)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def supprimer_commentaire(request, pk):
    try:
        ...
        commentaire.save()
        return Response({'message': 'Commentaire supprime'}, status=status.HTTP_200_OK)
    except Comment.DoesNotExist:
        return Response({'error': 'Commentaire introuvable ou non autorise'}, status=status.HTTP_404_NOT_FOUND)
...
def toggle_favori(request):
    ...
    if not prestatire_id:
        return Response({'error': 'prestatire_id est obligatoire'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if not created:
        favori.delete()
        return Response({'message': 'Favori retire', 'favori': False})

    return Response({'message': 'Prestataire ajoute aux favoris', 'favori': True}, status=status.HTTP_201_CREATED)
...
def mes_favoris(request):
    ...
    try:
        ...
        return Response({'error': 'Profil client introuvable'}, status=status.HTTP_404_NOT_FOUND)